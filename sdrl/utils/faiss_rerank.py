...
def _faiss_use_gpu():
    return os.environ.get('FAISS_USE_GPU', '').lower() in ('1', 'true', 'yes')
...
def compute_ranked_list(features, k=20, search_option=0, fp16=False, verbose=True):
    ...
    if search_option == 0:
        # Default CPU Faiss (see faiss_utils.search_raw_array_pytorch); use GPU when FAISS_USE_GPU=1
        res = None
        if _faiss_use_gpu():
